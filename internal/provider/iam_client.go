...
// iamClient calls the access policy endpoints directly, since the generated SDK
// does not cover them yet. It reuses the SDK client's host and credential.
...
// iamPolicy is the complete set of bindings on a resource. The etag is what
// makes concurrent applies safe: the server rejects a write built on a stale
// read rather than letting one run discard another's bindings.
type iamPolicy struct {
	// omitempty keeps the etag out of a rendered policy document, which is
	// built locally and was never read from a server.
...
// etag, retrying when another writer got there first. The _binding and _member
// resources exist so several configurations can manage one resource, so losing
// a race is expected rather than an error.
...
// setRole replaces the members of one role. A role left with no members drops
// its binding, since an empty binding and a missing one mean the same thing.
...
// normaliseMembers sorts and de-duplicates so a reordered list is not a change.
...
// canonicalisePolicy puts a policy into the one shape the data source and the
// policy resource both encode: bindings sorted by role, members sorted and
// de-duplicated, empty bindings dropped. The server orders bindings its own
// way, so without this every plan shows a diff.
